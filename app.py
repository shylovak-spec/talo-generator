...
def replace_headers_styled(doc, reps):
    """
    Виконує заміну тегів у тексті та робить ключові поля ЖИРНИМИ (п.4).
    """
    ...
    for p in doc.paragraphs:
        ...
        # 2. Форматування жирним (Тільки для ключів)
        for label in bold_labels:
            if label in p.text:
                full_text = p.text
                # Перевіряємо, щоб не ламати текст, якщо лейбла немає на початку
                if label in full_text:
                    p.clear()
                    parts = full_text.split(label, 1)
                    # Частина до лейблу (якщо є)
                    if parts[0]: 
                        p.add_run(parts[0])
                    # Лейбл (ЖИРНИМ)
                    run_b = p.add_run(label)
                    run_b.bold = True
                    # Частина після лейблу (ЗВИЧАЙНИМ)
                    if len(parts) > 1:
